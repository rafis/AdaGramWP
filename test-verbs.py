...
if __name__ == '__main__':
    ...
    #datasets = ['semeval-2007']

    for dataset in datasets:
        subprocess.call('./run.sh benchmark/semeval2010.jl %s 5 < datasets/%s/dataset_verbs.txt > __result__.tmp' % (model, dataset), shell=True)
        true_answers = read_answers('datasets/%s/key_verbs.txt' % dataset)
